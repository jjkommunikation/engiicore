...
export default function Edit({ attributes, setAttributes }) {
    ...
    const addPanel = () => {
        const newPanels = [...panels, { subtitle: "", imageUrl: "", iconUrl: "" }];
    ...
    return (
        <div {...useBlockProps()}>
            {panels.map((panel, index) => (
                <Panel>
                    <PanelBody
                        key={index} header={`Panel ${index + 1}`}
                        title={panel.subtitle ? panel.subtitle : 'Tab title'}
                    ...
                    >
                        ...
                        <PanelRow>
                            <Button isDestructive onClick={() => removePanel(index)}>
